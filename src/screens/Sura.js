    ...
    return (
        <SafeAreaView style={styles.container}>
            ...
            <ScrollView contentContainerStyle={styles.scrollViewer} >
                { suraAyat && suraAyat.map((aya, i) => {
                    return (
                        <Text style={styles.aya} key={`${surMeta[suraIndex].englishName}_${i}`}>{aya} <View style={styles.bg}></View><Text style={styles.ayaEnd}>{i+1}</Text></Text>
                    )
                })}
            </ScrollView>
        </SafeAreaView>
